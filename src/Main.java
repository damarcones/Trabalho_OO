...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		ArrayList<Veiculo> veiculos = new ArrayList<>(); // criando uma arraylist do tipo veiculo
		int opcaoMenu = -1;
					...
					do {
						try {
							...
							case 1: //REGISTRAR ABASTECIMENTO
								...
								try {
									veiculos.get(veiculos.size() - 1)
											.addAbs(RegistrarAbastecimento.init(veiculos.get(veiculos.size() - 1))); //adicionar uma despesa no arraylist de despesas
							...
							case 2: //REGISTRAR MANUTENCAO
								...
								try {
									veiculos.get(veiculos.size() - 1)
											.addMA(RegistrarManutencao.init(veiculos.get(veiculos.size() - 1))); //adiciona uma manutencao no arraylist de manutencao
							...
							case 3: //REGISTAR TAXA(IMPOSTO, SEGURO, ETC)
								...
								veiculos.get(veiculos.size() - 1).addtax(RegistrarTaxa.init());  //adicionar taxa no arraylist de taxa
						...
						try {
							...
							case 1: // gerar relatorio simples de abastecimento
								JOptionPane.showMessageDialog(null, RelatorioSimples
										.GerarRelatorioAbastecimento(veiculos.get(veiculos.size() - 1).getAbs()));
								break;

							case 2: // gerar relatorio simples de manutencao
								JOptionPane.showMessageDialog(null, RelatorioSimples
										.GerarRelatorioManutencao(veiculos.get(veiculos.size() - 1).getMA()));
								break;

							case 3: // gerar relatorio simples de taxa
								JOptionPane.showMessageDialog(null, RelatorioSimples
										.GerarRelatorioTaxa(veiculos.get(veiculos.size() - 1).getTAX()));
								break;
